...
public class SLCustomsDAO implements ActionConstants {
	...
	private static Logger logger = Logger.getLogger(SLCustomsDAO.class.getName());
...

			{
				if (!commonMethods.isNull(slcustomVO.getBranchCode())
					...
					if (!commonMethods.isNull(slcustomVO.getBranchCode())) {
						...
						System.out.println("branch code : " + slcustomVO.getBranchCode());
						branchSelectionQuery += " TRIM(UPPER(CABBN)) LIKE '"
					...
					if (!commonMethods.isNull(slcustomVO.getBranchCode())
						...
						branchSelectionQuery += SQL_AND + " TRIM(UPPER(FULLNAME)) LIKE '"
								+ slcustomVO.getBranchName().trim().toUpperCase() + "%' ";
					...
					if (!commonMethods.isNull(slcustomVO.getBranchCode())
						...
						branchSelectionQuery += SQL_AND + " TRIM(UPPER(CITY)) LIKE '"
								+ slcustomVO.getBranchCity().trim().toUpperCase() + "%' ";
	...
	public ArrayList<SLCustomVO> fetchInputBranchDetailsDAO(String branchQuery) {
		...
		try {
			...
			while (rs.next()) {
				...
				slCustomVO.setBrncode(rs.getString("CABBN").substring(1, 4).trim());
		...
		try {
...

			{
				...
				if (!commonMethods.isNull(slcustomVO.getBranchCode())
					...
					query += " TRIM(UPPER(BRANCH)) LIKE '" + slcustomVO.getBranchCode().trim().toUpperCase() + "%'"
				...
				if (!commonMethods.isNull(slcustomVO.getBranchCode())
					...
					query += " TRIM(UPPER(BRANCH)) LIKE '" + slcustomVO.getBranchCode().trim().toUpperCase() + "%' "
							+ SQL_AND + " TRIM(UPPER(SUBPRDTYPE)) LIKE '"
			...
			while (rs.next()) {
				 ...
				 * table.setECA(rs.getInt("ECA")); table.setFEP(rs.getInt("FEP"));
				 ...
				 * table.setDate(rs.getDate("DATE")); bd.getTable1().add(table);
				 */

				table.setBank(rs.getString("BANK"));

				table.setBranch(rs.getString("BRANCH"));

				table.setName(rs.getString("NAME"));

				table.setAdd1(rs.getString("ADD1"));

				table.setAdd2(rs.getString("ADD2"));

				table.setAdd3(rs.getString("ADD3"));

				table.setNICCRBR(rs.getString("NIC_CR_BR"));

				table.setVatNo(rs.getString("VATNO"));

				table.setBName(rs.getString("BNAME"));

				table.setBAdd(rs.getString("BADD"));

				table.setBCountry(rs.getString("BCOUNTRY"));

				table.setECA(rs.getInt("ECA"));

				table.setFEP(rs.getInt("FEP"));

				table.setCurrCode(rs.getString("CURRCODE"));

				table.setAmount(rs.getBigDecimal("AMOUNT"));

				table.setToP(rs.getString("TOP"));

				table.setToD(rs.getString("TOD"));

				table.setRefNo(rs.getString("REFNO"));

				table.setLCNo(rs.getString("LCNO"));

				// table.setDate(rs.getDate("DATE"));
				if (rs.getDate("Date") != null) {
					table.setDate(dateFormat.format(rs.getDate("Date")));
				}
			...
			sb.append(CommonMethods.getXmlFromObject(bd));
	...
	public void updateRowStatus(int id) {
		...
		try {
			...
			DBConnectionUtility.surrenderDB(conUpdateRowStatus, psUpdateRowStatus, null);
		}
