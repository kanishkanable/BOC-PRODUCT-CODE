...
GetCustomPageHdrStrip("Edit Form","(Select Workflow, Branch & then ID to Edit Forms)",getUrlParam("RefID"));
...
$(document).on('click', '#lookup', function () {
	...
	//var processId= $(this).closest('tr').find('td:eq('+$($('th:contains("ProcessID")')[0]).index()+')').text();
	//var activityID= $(this).closest('tr').find('td:eq('+$($('th:contains("ActivityID")')[0]).index()+')').text();
	var FormID= $(this).closest('tr').find('td:eq('+$($('th:contains("FormID")')[0]).index()+')').text();
	var activityID='';
	...
	RedirectURL = IOP+"/ThemePro_LSW/"+LoadFrmXML("PG009")+"?activityId="+activityID+"&processId="+processId +"&View=FormEdit&FormID="+FormID;
